...
* What is this about?
 ...
 This is a post to generate and document the ~pytest.ini~ configuration I use for testing.

* Requirements
** The Neurotic Repository

To make the ~neurotic~ code (the code from this repository) importable you need to install the package (in development mode, since it's (hopefully) always changing) using [[https://pip.pypa.io/en/stable/][pip]].

#+begin_src fish
pip install --editable .
#+end_src

This is run at the top of the repository, above the ~neurotic~ folder with the code.
...
#+begin_notecard
The ~looponfail~ option loops over the failed tests when files change and then re-runs /all/ the tests once the failed tests pass. Since I combine it with ~failed-first~ this means it just re-runs the first failed test when a file changes. According to the ~pytest-xdist~ documentation, this option is deprecated but, as noted [[https://github.com/pytest-dev/pytest-xdist/issues/826][on github]], they don't have a replacement for it. It sounds like they want to add it to ~pytest~ itself instead of keeping it in the ~pytest-xdist~ plugin, but it hasn't happened yet. So for now, it's still one of the options.
#+end_notecard

** The Run-Once
This line is commented out and only uncommented when I need it (commenting out the options I normally use). It removes the automatic re-running of tests. I don't really use this here, but sometimes it can be helpful, especially when using something like Selenium that slows everything down a lot, to not re-run the test every time you save a file.
...
This is another line that I normally leave commented out, although I use it on occasion. ~pytest~ will grab standard out by default, making it impossible to run an interactive debugger. Sort of. They have support for python's [[https://docs.python.org/3/library/pdb.html][pdb]] debugger, but I use [[https://documen.tician.de/pudb/][pudb]] instead, so I use this set of arguments to turn off the capturing of standard out which lets me run the tests with PUDB. There is [[https://github.com/wronglink/pytest-pudb][a project]] that integrates ~pudb~ into pytest, but it appears to have died out, so I'll just stick to my old way of doing it.
