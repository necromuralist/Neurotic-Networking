...
#+OPTIONS: ^:{}
#+TOC: headlines 3
#+PROPERTY: header-args :session ~/.local/share/jupyter/runtime/kernel-7fa2ac72-2845-4353-8b87-4ca474cb81f2-ssh.json
...
* The Dataset
** The MNIST Dataset
The training images we will be using are from a dataset called the [[http://yann.lecun.com/exdb/mnist/][MNIST]] (Modified National Institute of Standards and Technology) database. The dataset contains 60,000 images of handwritten digits, from 0 to 9.

The images are 28 pixels x 28 pixels in size. The small size of its images makes MNIST ideal for simple training. Additionally, these images are also in black-and-white so only one dimension, or "color channel", is needed to represent them (as opposed to RGB with an alpha channel which would need four dimensions). Pytorch has a [[https://pytorch.org/vision/0.8/datasets.html#mnist][version of it]] ready-made for their system so we'll use theirs.
