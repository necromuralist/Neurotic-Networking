(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("d01a239b-ed28-43cd-b2e2-7c75f14c8323");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid 'd01a239b-ed28-43cd-b2e2-7c75f14c8323' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.pydata.org/bokeh/release/bokeh-1.0.4.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-widgets-1.0.4.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-tables-1.0.4.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-gl-1.0.4.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"1c8d38d0-aad1-427d-bfc2-1f8069e49cb5":{"roots":{"references":[{"attributes":{"below":[{"id":"1197","type":"LinearAxis"}],"left":[{"id":"1202","type":"LinearAxis"}],"min_border_bottom":10,"min_border_left":10,"min_border_right":10,"min_border_top":10,"renderers":[{"id":"1197","type":"LinearAxis"},{"id":"1201","type":"Grid"},{"id":"1202","type":"LinearAxis"},{"id":"1206","type":"Grid"},{"id":"1216","type":"BoxAnnotation"},{"id":"1227","type":"GlyphRenderer"}],"title":{"id":"1188","type":"Title"},"toolbar":{"id":"1212","type":"Toolbar"},"x_range":{"id":"1185","type":"Range1d"},"x_scale":{"id":"1193","type":"LinearScale"},"y_range":{"id":"1186","type":"Range1d"},"y_scale":{"id":"1195","type":"LinearScale"}},"id":"1189","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"1211","type":"ResetTool"},{"attributes":{},"id":"1203","type":"BasicTicker"},{"attributes":{"fill_alpha":0.2,"fill_color":"#1f77b4","line_alpha":0.2,"line_color":"black","x":{"field":"x"},"y":{"field":"y"}},"id":"1226","type":"Patch"},{"attributes":{"overlay":{"id":"1216","type":"BoxAnnotation"}},"id":"1210","type":"BoxZoomTool"},{"attributes":{"data_source":{"id":"1221","type":"ColumnDataSource"},"glyph":{"id":"1224","type":"Patch"},"hover_glyph":null,"muted_glyph":{"id":"1226","type":"Patch"},"nonselection_glyph":{"id":"1225","type":"Patch"},"selection_glyph":null,"view":{"id":"1228","type":"CDSView"}},"id":"1227","type":"GlyphRenderer"},{"attributes":{"plot":null,"text":"Word Counts Per Line Distribution"},"id":"1188","type":"Title"},{"attributes":{"dimension":1,"grid_line_color":{"value":null},"plot":{"id":"1189","subtype":"Figure","type":"Plot"},"ticker":{"id":"1203","type":"BasicTicker"}},"id":"1206","type":"Grid"},{"attributes":{},"id":"1209","type":"WheelZoomTool"},{"attributes":{"fill_alpha":0.5,"fill_color":"#1f77b4","x":{"field":"x"},"y":{"field":"y"}},"id":"1224","type":"Patch"},{"attributes":{},"id":"1208","type":"PanTool"},{"attributes":{"fill_alpha":0.5,"fill_color":"#1f77b4","line_alpha":1,"line_color":"black","x":{"field":"x"},"y":{"field":"y"}},"id":"1225","type":"Patch"},{"attributes":{},"id":"1230","type":"BasicTickFormatter"},{"attributes":{},"id":"1193","type":"LinearScale"},{"attributes":{"callback":null,"end":0.0788474854210965,"reset_end":0.0788474854210965,"reset_start":0.0,"tags":[[["line_counts_density","Density",null]]]},"id":"1186","type":"Range1d"},{"attributes":{"axis_label":"Density","bounds":"auto","formatter":{"id":"1232","type":"BasicTickFormatter"},"major_label_orientation":"horizontal","plot":{"id":"1189","subtype":"Figure","type":"Plot"},"ticker":{"id":"1203","type":"BasicTicker"}},"id":"1202","type":"LinearAxis"},{"attributes":{},"id":"1195","type":"LinearScale"},{"attributes":{"callback":null,"end":367.0147832050558,"reset_end":367.0147832050558,"reset_start":-3.014783205055803,"start":-3.014783205055803,"tags":[[["line_counts","line_counts",null]]]},"id":"1185","type":"Range1d"},{"attributes":{},"id":"1207","type":"SaveTool"},{"attributes":{},"id":"1232","type":"BasicTickFormatter"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"1187","type":"HoverTool"},{"id":"1207","type":"SaveTool"},{"id":"1208","type":"PanTool"},{"id":"1209","type":"WheelZoomTool"},{"id":"1210","type":"BoxZoomTool"},{"id":"1211","type":"ResetTool"}]},"id":"1212","type":"Toolbar"},{"attributes":{},"id":"1239","type":"UnionRenderers"},{"attributes":{"axis_label":"line_counts","bounds":"auto","formatter":{"id":"1230","type":"BasicTickFormatter"},"major_label_orientation":"horizontal","plot":{"id":"1189","subtype":"Figure","type":"Plot"},"ticker":{"id":"1198","type":"BasicTicker"}},"id":"1197","type":"LinearAxis"},{"attributes":{},"id":"1198","type":"BasicTicker"},{"attributes":{"callback":null,"renderers":[{"id":"1227","type":"GlyphRenderer"}],"tooltips":[["line_counts","@{line_counts}"],["Density","@{line_counts_density}"]]},"id":"1187","type":"HoverTool"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[200]},"y":{"__ndarray__":"","dtype":"float64","shape":[200]}},"selected":{"id":"1222","type":"Selection"},"selection_policy":{"id":"1239","type":"UnionRenderers"}},"id":"1221","type":"ColumnDataSource"},{"attributes":{"source":{"id":"1221","type":"ColumnDataSource"}},"id":"1228","type":"CDSView"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"1216","type":"BoxAnnotation"},{"attributes":{"grid_line_color":{"value":null},"plot":{"id":"1189","subtype":"Figure","type":"Plot"},"ticker":{"id":"1198","type":"BasicTicker"}},"id":"1201","type":"Grid"},{"attributes":{},"id":"1222","type":"Selection"}],"root_ids":["1189"]},"title":"Bokeh Application","version":"1.0.4"}}';
                  var render_items = [{"docid":"1c8d38d0-aad1-427d-bfc2-1f8069e49cb5","roots":{"1189":"d01a239b-ed28-43cd-b2e2-7c75f14c8323"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.pydata.org/bokeh/release/bokeh-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.pydata.org/bokeh/release/bokeh-1.0.4.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.pydata.org/bokeh/release/bokeh-widgets-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.pydata.org/bokeh/release/bokeh-widgets-1.0.4.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.pydata.org/bokeh/release/bokeh-tables-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.pydata.org/bokeh/release/bokeh-tables-1.0.4.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();