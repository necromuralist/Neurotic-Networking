...
%autoreload 2
#+END_SRC
#+PROPERTY: header-args :session ~/.local/share/jupyter/runtime/kernel-d49dae16-d566-49f7-8773-a70722f3a741-ssh.json
...
#+end_src
** The Data
We'll be using data that's part of the [[https://github.com/rwightman/pytorch-image-models][git repository for ~timm~ ]]. Once you clone the repository the file within it that we want will be ~results/results-imagenet.csv~. This is the result of using the [[https://github.com/rwightman/pytorch-image-models/tree/main/results#imagenet-validation---results-imagenetcsv][Imagenet Validation]] set to validate the models.
...
*** Get Data
We're going to merge our "validation" data with two "benchmark" files (also in the "results" folder) doing some cryptic filtering and data wrangling. It's not obvious what everything is doing so let's use it first and maybe figure it out later.
...
We're going to be doing some plotting so here are some helper functions.
...
Let's look at what was added to our original validation data.
...
#+begin_src python :results output :exports both
counts = inference.family.value_counts().to_frame().reset_index().rename(columns = {"index": "Family", "family": "Count"})
print(TABLE(counts, headers="keys", showindex=False))
#+end_src
...
chart = altair.Chart(counts).mark_bar().encode(
    x="Count", y=altair.Y("Family", sort="-x"),
).properties(
    width=PLOT_WIDTH,
    height=PLOT_HEIGHT,
