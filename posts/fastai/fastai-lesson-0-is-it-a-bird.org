...
def predict_category(path: str, learner):
    ...
    with learner.no_bar():
        prediction,_, probabilities = learner.predict(
            PILImage.create(path))
    print(f"This is a {prediction}.")
    print(f"Probability it's a bird: {float(probabilities[0]):.2f}")
    return

predict = partial(predict_category, learner=learner)
#+end_src

Let's look at a couple of examples to see how to use the model. First, a crow.
...
Here we see a problem with machine learning models (and humans too) - they need to make a classification even if they have no idea what the immage is. It's 100%  sure that it's a bird, but since it was only trained on two categories it might be truer to say that it's 100% sure that it isn't a forest. Even if it's true that the plane has enough attributes in common with a bird to say that it's ambiguous, the certainty of the model seems problamatic.
