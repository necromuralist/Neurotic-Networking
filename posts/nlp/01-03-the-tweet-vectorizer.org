...
#+OPTIONS: ^:{}
#+TOC: headlines 2
#+PROPERTY: header-args :session ~/.local/share/jupyter/runtime/kernel-5fb2fe5e-41b5-43a3-8dd6-fc68323fec92.json
...
#+END_SRC
* Beginning
  In the previous post ({{% doc %}}01b-twitter-word-frequencies{{% /doc %}}) I built up a word-counter now we're going to use it to create word-counters for our tweets.
...
vectorizer.reset()
vectorizer.tweets = test.tweet
test_data = pandas.DataFrame(vectorizer.vectors,
...
We also need to use the vectorizer to vectorize future tweets so I'll pickle that too.
...
Next up in the series: {{% doc %}}01d-implementing-twitter-logistic-regression{{% /doc %}}.
