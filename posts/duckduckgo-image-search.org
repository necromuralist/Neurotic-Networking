...
I'll show a request for "rabbit lop" using URL parameters. The actual code is using the [[https://requests.readthedocs.io/en/latest/][requests]] library and sends them as a payload dictionary instead, but I thought it might be more familiar to see them as URL parameters (and I it makes it so you can copy and paste the output into a browser to see what happens).

The first thing that ~ddg_images~ does is make a POST request to "https://duckduckgo.com/" using the search terms as data.
...
The "vqd" is one of the arguments to the ~DDG.deep_initialize~ function in that javascript and the ~duckduckgo-search~ code extractis it using substring searching, giving us just the vqd.

#+begin_example
vqd=3-175223187338608511244788076450682226312-294342034741290420994864096532891255767
#+end_example

Then a second request is made using the VQD token, and whatever other parameters you want - in this case we're setting the region (~l=wt-wt~ meaning "no region"), asking that the response be JSON (~o=json~), using the  keywords "rabbit lop" (~q="rabbit lop"~), and turning safe-search off (~p=-1~). To make it work you also need to send the request to a special endpoint ~https://duckduckgo.com/i.js~. So we end up with a request that looks like this:
...
The payload to the response is a JSON blob that gets converted by the python code into a dictionary (if you past the example url into a browser address bar you should be able to see the response).
...
vqd_index_start = response.content.index(b"vqd='") + 5
vqd_index_end = response.content.index(b"'", vqd_index_start)
