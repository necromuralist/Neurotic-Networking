...
def train_only(model:nn.Module, optimizer: object, criterion: object,
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_batches:
            ...
            optimizer.step()
            total_loss += loss.item()
            train_losses.append(total_loss/len(inputs))
    return pandas.DataFrame.from_dict({"Training Loss":train_losses})


def test_only(model:nn.Module,
              test_batches: DataLoader,
              device: object=None) -> float:
    """Tests the accuracy of the model

    Args:
     model: pre-trained model
     test_batches: iterator of test data
